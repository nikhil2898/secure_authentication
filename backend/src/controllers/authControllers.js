...
import { getForgotPasswordHtml } from "../templates/getForgotPasswordHtml.js";
...
export const loginUser = async (req, res) => {
  ...
  return res.status(200).json({
    message:
      "If you email is valid, an OTP is sent to your email. It will expire in 5 min",
  });
};
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    const rateLimitKey = `reset_password ${req.ip}:${email}`;
    ...
    const resetKey = `reset_password:${resetToken}`;
     ...
     await redisClient.set(resetKey, dataToStore, { EX: 300 });

    const html = getForgotPasswordHtml({ email, token: resetToken });

    await sendForgotPassword(email,html);

    await redisClient.set(rateLimitKey, "true", { EX: 60 });

    return res.status(200).json({
      message:
        "If your email is valid, a password reset link has been sent. It will expire in 5 min",
    });
  } catch (err) {
    return res.status(500).json({
      message: "Internal Server Error",
...
export const resetPassword = async(req,res) => {
    try{
      ...
      const resetToken = `reset_password:${token}`;

      const userDataJson = await redisClient.get(resetToken);
